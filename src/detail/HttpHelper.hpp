...
public:
    ...
    static auto post3(std::string_view url, const AuthenticationToken &token,
                      const model::playlistdata &pD, bool include_bearer = true) -> std::string;
    static auto get1(std::string_view url) -> std::string;
    static auto get2(std::string_view url, const AuthenticationToken &token, bool include_bearer) -> std::string;
    static std::string put1(std::string_view url, const AuthenticationToken &token,
                            const std::map<std::string, std::string> &postData, bool include_bearer);
    static std::string put2(std::string_view url, const AuthenticationToken &token,
                            const model::playlistdata &pD, bool include_bearer);
    static std::string delete_(std::string_view url, const AuthenticationToken &token, bool include_bearer);
};
