...
Page<Album> Album::search(std::string &albumName,
...
Album::Album(const model::album &t_album) : _availableMarkets(t_album.available_markets),
...
Album::Album(model::album &&t_album) noexcept : _availableMarkets(std::move(t_album.available_markets)),
...
Page<Track> v1::Album::get_album_tracks(std::string_view album_id, int limit, int offset) {
    auto local_future = stlab::async(stlab::default_executor, detail::HttpHelper::get1,
                                     "https://api.spotify.com/v1/albums/" + std::string(album_id)
...
std::vector<Album> Album::get_albums(const std::vector<std::string> &album_ids) {
    auto local_future = stlab::async(stlab::default_executor, detail::HttpHelper::get1,
