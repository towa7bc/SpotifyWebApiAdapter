...
Page<Playlist> Browse::get_featured_playlists(const AuthenticationToken &token, std::string_view locale,
    ...
    auto local_future = stlab::async(stlab::default_executor, detail::HttpHelper::get2, queryString, token, include_bearer)
                        | ([](std::string_view s) { return json_t::parse(s); })
                        | ([](const json_t &j) { return j.get<model::featured_playlist_search_result>(); });
...
Page<Album> Browse::get_new_releases(const AuthenticationToken &token, std::string_view country, int limit, int offset) {
