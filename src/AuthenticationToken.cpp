...
auto AuthenticationToken::setAccessToken(std::string_view mAccessToken) -> void {
    _access_token = mAccessToken;
}

auto AuthenticationToken::getMExpiresOn() const -> const boost::posix_time::ptime & {
    return _expires_on;
}

auto AuthenticationToken::setExpiresOn(const boost::posix_time::ptime &mExpiresOn)
        -> void {
    _expires_on = mExpiresOn;
}

auto AuthenticationToken::refresh() -> void {
    auto auth_token_future = stlab::async(stlab::default_executor, Authentication::get_access_token, _refresh_token);
