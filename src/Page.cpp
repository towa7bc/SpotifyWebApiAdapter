...
Page<T> Page<T>::get_next_page() {
    ...
    auto local_future = stlab::async(stlab::default_executor, detail::HttpHelper::get1, _next);
...
Page<T> Page<T>::get_previous_page() {
    ...
    auto local_future = stlab::async(stlab::default_executor, detail::HttpHelper::get1, _previous);
    json_t j_obj = json_t::parse(stlab::blocking_get(local_future));
