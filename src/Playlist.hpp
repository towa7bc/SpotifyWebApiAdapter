...
public:
    ...
    static Page<Playlist> get_playlist(const User &user,
                                       std::string_view playlist_id,
                                       const AuthenticationToken &token);
    static Page<PlaylistTrack> get_playlist_tracks(std::string_view user_id,
                                                   std::string_view playlist_id,
                                                   const AuthenticationToken &token);
    [[nodiscard]] Page<PlaylistTrack> get_playlist_tracks(const AuthenticationToken &token) const;
    void add_track(const Track &track, AuthenticationToken &token) const;
    void add_tracks(const std::vector<Track> &track_uris, AuthenticationToken &token) const;
